...
- *(2026-07-04, uk)* **`self-scoping-formatter-composition`** — a shared
  per-field formatter (a range renderer, a month/weekday namer, a clock-time
  printer) sometimes returns a BARE value ("March", "9 p.m.") and sometimes,
  for a different underlying shape (a range, a special pinned value like
  midnight), returns an already-self-contained phrase that carries its OWN
  connective or omits a preposition entirely ("from January through March
  inclusive"; a preposition-less "midnight"). A caller written against the
  bare case wraps it unconditionally — prepending "in", "of the month", or a
  restated second cadence — while a sibling caller of the very same helper,
  written against the range/special case, already special-cases it
  correctly. Nearly every bug in this run traced to exactly this: the
  formatter's return shape is polymorphic and undeclared, so some call sites
  got the memo and others didn't (a leading-cadence confinement getting both
  the confinement AND a restated second cadence; a preposition doubled onto
  a range phrase that already supplies its own; a preposition glued onto a
  bare adverb that takes none). *Question:* does any formatter your renderer
  calls change its OUTPUT SHAPE (bare value vs. self-contained connective
  phrase vs. preposition-less special case) depending on the input's shape
  (single/range/list/step/pinned value) — and if so, does EVERY caller
  branch on that same shape signal, or does at least one caller assume a
  single fixed shape and wrap blindly? *Resolve:* either (a) make the
  formatter's contract shape-invariant (always bare, always self-contained)
  so callers never need to know, or (b) expose the shape as an explicit
  return signal (a tag, a boolean) so every caller can branch correctly —
  never leave "which shapes are already self-contained" as tribal knowledge
  re-derived ad hoc at each call site. *Detector:* for every helper that
  renders a field value, generate cases across all of that field's shapes
  (single, list, range, step, special/pinned values) and assert no caller's
  composed output contains a doubled connective/preposition or a
  contradicting second cadence for any shape.

- *(2026-07-04, uk)* **`lexicon-literal-string-integrity`** — a language's
  literal lookup tables (month/weekday name tables, leading/trailing marker
  words) can ship two kinds of silent corruption that no amount of
  phrase-construction panel review will catch, because both look almost
  right on a skim: (1) a homoglyph typo — a Latin letter substituted for a
  visually identical native-script letter; (2) an unresolved developer
  placeholder shipped verbatim as output — two alternative forms joined by a
  literal separator that was meant to be disambiguated by logic that was
  never written (e.g. a masculine/feminine table cell left as
  "form1/form2" and rendered as-is instead of picking one). Both produce
  garbage/broken text for every user who hits that exact table cell, and
  both are invisible to a reading panel that never happens to probe that
  one cell. *Question:* does your language's lexicon module contain any
  literal string that isn't pure target-script characters, or that still
  contains an unresolved separator (`/`, `|`, " or ") between two candidate
  forms? *Resolve:* run a mechanical script-purity check (every string
  matches the language's expected Unicode block) AND a placeholder check
  (no alternation-separator characters inside any lexicon string) over the
  *whole* lexicon table, not just the cells a corpus pattern happens to
  exercise. *Detector:* a lint pass over every literal string constant in
  `src/lang/<code>/` asserting it matches the language's script-range regex
  and contains no `/`/`|`/" or " alternation marker.

- *(2026-06-22, en)* **`range-wrap-through-zero`** — when an exclusive upper
  bound is computed arithmetically and wraps through zero (e.g., hour 23 + 1
  mod 24 = 0, displayed as midnight), the prose shows a chronologically *earlier*
  time as the endpoint than the last actual fire ("from 1 a.m. until midnight"
  implies wrapping back through the night). The existing `range-boundary` trap
  covers the inclusive-vs-exclusive choice but not this case. *Question:* if your
  renderer ever displays the arithmetic exclusive end, does it guard for wrap-
  through-zero and substitute the inclusive last-fire time instead? *Resolve:*
  when the exclusive-end value is at or below the start value (in a forward-only
  field like hours), switch to inclusive last-fire display ("from 1 a.m. through
  11 p.m.") or rephrase as "from X onwards". Apply consistently to all stepped
  and range fields that can wrap. *Detector:* for every hour-range pattern whose
  exclusive end wraps, assert the endpoint in the output is not ≤ the start time
  expressed as a clock value.

- *(2026-06-22, en)* **`bare-juxtaposition-ambiguity`** — when a DOM ordinal (or
  any positional value) is followed directly by its scope qualifier without a
  preposition, the qualifier is PP-attachment-ambiguous: it reads as appositive
  to the preceding value rather than as an independent scope phrase. "on the 1st
  January through March" has two parses — [on the 1st January] [through March]
  (British date reading + trailing extent) vs. [on the 1st] [January through
  March] (intended). Sibling renderer paths that include a preposition ("of every
  other month") prove the bug is a dropped `of`/`in`, not an unsolvable ambiguity.
  *Question:* does your renderer ever juxtapose a DOM/value and its scope
  qualifier without a preposition? *Resolve:* insert the correct preposition
  ("of", "in", "from") between the value and its qualifier; or front the qualifier
  so the ordinal trails ("in January through March, on the 1st"). *Detector:*
  grep the corpus for ordinal-month adjacency (e.g., `\d+(st|nd|rd|th)\s+[A-Z]`);
  every hit should carry a preposition between the ordinal and the month name.

- *(2026-06-22, en)* **`garden-path-trailing-day-qualifier` — list extension** —
  the 2026-06-21 entry's detector covers hour-range / window phrases ("for every
  pattern with an hour-range or time-window"). The same garden-path stranding
  occurs when a day qualifier trails a *listed* hour enumeration: "at 5 and 10
  minutes and 30 seconds past 9 a.m., 5 p.m., 7 p.m., 9 p.m. and 11 p.m. **on
  Mondays**" — "on Mondays" syntactically attaches to "11 p.m." before the reader
  can reparse it as governing the whole set. *Extend the detector:* also assert
  that when the hour field is a multi-item list AND a date/weekday qualifier is
  present, the qualifier appears before the "past …" or listed-hours phrase in the
  output. The window path and the list path must apply the same fronting rule.
