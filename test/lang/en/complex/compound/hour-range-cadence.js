...
// 9 a.m. through 5 p.m." (the pinned minute is restricted, so the run is
// discrete; any non-contiguous hour is appended with "and at Z").
...
describe('Hour range under a fixed minute and a second reads as a window:',
  function() {
    ...
    // A range inside a list: the contiguous span is the "through" window (the
    // pinned minute is restricted, so the run is not continuous to the top of
    // the next hour), and the non-contiguous hour is appended with "and at Z".
    ...
    // the restricted minute makes the run discrete, so the window reads the
    // bare "through <last hour>" span ("through 5 p.m."), never a closing :05
    // that would read as a span contradicting the minute clause.
